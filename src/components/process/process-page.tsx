...
const ProcessPage = () => {
    ...
    return (
        <main ref={sectionRef} className="process-page py-32 px-8 max-w-screen-xl mx-auto mt-20">
            <div className="process-header text-center mb-24">
                <h1 className="text-6xl font-bold mb-6 text-gradient">From Concept to Creation</h1>
            ...
            <div ref={timelineRef} className="timeline relative max-w-4xl mx-auto">
                <div className="timeline-line absolute top-0 left-1/2 -translate-x-1/2 w-1 h-full bg-gray-200">
                     <div className="timeline-line-progress w-full h-full bg-primary origin-top" />
                </div>

                {processStages.map((stage, index) => (
                    <div key={index} className="timeline-item relative w-full mb-16 flex justify-center items-center">
                        <div className={`w-full md:w-1/2 flex ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
                            <div className="timeline-content w-full md:max-w-md bg-white rounded-2xl p-8 shadow-lg border border-gray-200/80">
                                <span className="text-5xl font-bold text-primary/10 absolute -top-4 -left-4">{stage.number}</span>
                                <h2 className="text-3xl font-bold mb-4 text-primary relative">{stage.title}</h2>
